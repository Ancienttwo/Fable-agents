...
agent 和路由章节只装一次、全局生效,机器上的每个项目都能用。`--project` 只用来指定冒烟测试在哪个项目目录下运行(默认:当前目录),不影响安装位置。脚本是幂等的——可以放心重复执行。

参数:
- `--project <dir>` — 冒烟测试运行所在的项目目录(默认:当前目录)
...
> 从 https://github.com/Ancienttwo/Fable-agents 安装这套模型路由设置——克隆仓库,运行 `bash scripts/install.sh`,然后报告每一层的结果(`[new]`/`[ok]`/`[conflict]`/`[warn]`)。遇到 `[conflict]` 时绝不覆盖任何文件。

## 它安装什么

1. 把 `agents/fast-worker.md`、`agents/deep-reasoner.md`、`agents/gatekeeper.md` 安装进你的全局 Claude 目录(`$CLAUDE_CONFIG_DIR/agents`,默认 `~/.claude/agents`)——每个项目都能用
...
- **`deep-reasoner`**(Opus,`effort: max`)——架构研究、高难度推理、高风险判断。只负责给建议,最终框架由编排者确认。返回 `RECOMMENDATION: <一句话> — confidence: HIGH/MEDIUM/LOW`。
- **`fast-worker`**(Sonnet,`effort: max`)——实现、测试、重构、文档、机械性执行。返回 `RESULT: DONE/PARTIAL/BLOCKED` 及验证输出。
...
assets/deep-reasoner.md           安装进 ~/.claude/agents(全局)的 agent 定义
assets/fast-worker.md             安装进 ~/.claude/agents(全局)的 agent 定义
assets/gatekeeper.md              安装进 ~/.claude/agents(全局)的 agent 定义
